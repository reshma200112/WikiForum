...
<em class="fa fa-history"></em><span>Show activity history</span>
            ...
            <div class="question-crud-div">
              ...
              <!-- 1 for current user posted question and 2 for user who have high reputation score -->
              <button @click="deleteQuestion" class="delete-question-btn" v-if="checkUserForDelete==1">
                <em class="fa fa-trash" aria-hidden="true">Delete</em>
              </button>
              <button @click="closeQuestion" class="delete-question-btn" v-if="checkUserForDelete==2">
                <em class="fa fa-trash" aria-hidden="true">Close</em>
              </button>
            </div>
  ...
  <button id="recentlyadded" class="btnSort" @click="onSortChange(2)">Like <v-tooltip activator="parent" location="bottom"
            ...
            <div :key="item.id" v-for="(item, index) in answers" class="div-verify">
              ...
              <div elevation="6" outlined shaped class="v1" id="answerCard">
                <!--  -->
                <div class="answer-topDiv" :id="'answerfocus-' + item.id">
                ...
                <div class="user-info" style="max-width:200px">
                  ...
                  <div class="user__comment__reaction"  v-bind:id="`Answercomments${item.id}`">
                    ...
                    <div class="answer-history" @click="showHistoryAnswer()">
...
><span class="mdi mdi-plus"></span>Add</v-btn>
            ...
            <form action="" name="addAnswer" @submit.prevent="addAnswer()" id="addAnswer-media">
...
export default {
  ...
  computed:{
    checkUserForDelete()
    {
      if(this.currentUser.id==this.questionUser)
      {
        return 1;
      }
  ...
  methods: {
    ...
    async showHistoryAnswer(){
     await this.$router.push('/answerhistory')
    },
    async addAnswer() {
...
//   const targetElement = document.querySelector('#Questioncomments');
...
alertmsg=alertmsg+"<br><small>Your reputation score will be deducted!!</small>";
  ...
  if (result.isConfirmed) {
      ...
      http
          ...
          .then(() => { this.$router.push("/dashboard").then(()=>{
  ...
  if (result.isConfirmed) {
      ...
      http
          .put("/deleteQuestion/" + questionId)
          .then(() => { this.$router.push("/dashboard").then(()=>{
            this.toast.success("Question closed successfully", {
              position: POSITION.TOP_CENTER,
              timeout: 1700,
            });
          })
...
if(
 ...
 async beforeMount() {
  ...
  const answerId = localStorage.getItem('answerId');
    ...
    const focusanswer_id = localStorage.getItem('focusanswer_id');
    if (focusanswer_id) {
      const answerDivFocus = document.getElementById('answerfocus-' + focusanswer_id);
      if (answerDivFocus) {
        answerDivFocus.scrollIntoView({ behavior: 'smooth',block:'center'});
        localStorage.removeItem('focusanswer_id');
      }
    }
  },
};
